...
def predict(img: Image.Image, top_k: int):
...
if uploaded:
    ...
    run = col1.button("Predict", type="primary", use_container_width=True)
    save_btn = col2.button("Save to History", use_container_width=True)

    if run:
        preds = predict(img, top_k)
